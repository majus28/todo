        ...
        if (this.props.propsData.is_edit && this.props.propsData.editItem.id === item.id) {
            return (
                <div key={item.id}>
                    <Flex p={2} align='center'>
                        ...
                        <Box px={1} w={2 / 2}>
                            <RaisedButton className='right gap' backgroundColor="rgb(0, 0, 0)"
                                          onClick={() => this.cancel()}>Cancel</RaisedButton>
                            <RaisedButton className='right gap' backgroundColor="rgb(0, 104, 212)"
                                          disabled={this.props.propsData.disabled}
                                          onClick={() => this.updateTodo(item.id)}>{this.props.propsData.btnText}</RaisedButton>
                        </Box>
                    </Flex>
                </div>
            )
            ...
            return (
                <div key={item.id}>
                    <Flex p={2} align='center'>
                        <Box px={2} w={1 / 2}
                             className={'text ' + (item.status ? item.status.toLowerCase() : 'created')}
                             onClick={() => this.statusChange(item.id)}>
                            <span data-tip data-for='delete' key={item.id}>{item.note}</span>
                        </Box>
                        <Box px={1} w={2 / 2}>
                            <RaisedButton className="right gap" backgroundColor="rgb(189, 7, 7)"
                                          onClick={() => this.delete(item.id)}>Delete</RaisedButton>
                            <RaisedButton className='right gap' backgroundColor="rgb(0, 104, 212)"
                                          onClick={() => this.update(item.id)}>Edit</RaisedButton>
                        </Box>
                    </Flex>
                </div>
            )
